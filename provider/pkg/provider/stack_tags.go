...
// stackTagClient mocks the interface
type stackTagClient interface {
	CreateStackTag(ctx context.Context, stackName pulumiapi.StackName, stackTag pulumiapi.StackTag) error
	GetStackTag(ctx context.Context, stackName pulumiapi.StackName, tagName string) (*pulumiapi.StackTag, error)
	DeleteStackTag(ctx context.Context, stackName pulumiapi.StackName, tagName string) error
}
...
func (st *PulumiServiceStackTagResource) Create(req *pulumirpc.CreateRequest) (*pulumirpc.CreateResponse, error) {
	...
	err = st.client.CreateStackTag(ctx, stackName, stackTag)
	if err != nil {
		return nil, err
	}
